...
- **Ollama連携 (gemma3:4b)**: ローカルLLM を使用した画像説明の自動生成、Markdown整形・要約、スクレイピング戦略分析を統合
- **3種類のファイル出力**: 元データ・整形済・要約済の3ファイルを自動生成（Ollama利用時。元データとの比較で整形による情報損失をチェック可能）
...
| 要約済 | `ファイル名_要約済_YYYYMMDDHHmmss.md` | Ollamaで要約したデータ |
...
| 要約済 | `ファイル名_要約済_YYYYMMDDHHmmss.md` | Ollamaで要約したMarkdown |

処理中はオーバーレイに現在の進行状況（依存パッケージ確認 → Playwrightスクレイピング → Ollama JSON整形 → Ollama要約）がリアルタイムで表示されます。
...
- **自動要約**: 変換結果・スクレイピング結果をOllamaで自動要約し、Markdownファイルとして出力します
...
4. **完了**
   ...
   - ファイルをドロップすると、元データ・整形済・要約済の3ファイルが自動生成されます
