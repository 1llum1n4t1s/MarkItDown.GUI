...
- **Webスクレイピング**: URLを入力するだけでWebページの内容をJSON形式で抽出・保存
  ...
  - X/Twitter: ユーザーのタイムライン全件取得＋画像オリジナル画質ダウンロード（一時セッションで安全に実行）
  - Instagram: ユーザーの全投稿メディア（画像・動画）を一括ダウンロード（一時セッションで安全に実行）
...
- **安定した依存環境**: インストール済みのPythonパッケージは通常起動・通常実行時に自動更新せず、未導入時のみセットアップ
...
| X/Twitter | Playwright（専用スクリプト） | ユーザータイムライン全件取得、画像オリジナル画質DL、一時セッション利用 |
| Instagram | Instaloader（専用スクリプト） | 全投稿メディアDL、一時セッション利用 |
...
4. 既定ではセッション情報は終了時に破棄されます。永続保存が必要な場合のみ `appsettings.xml` の `AllowSocialSessionPersistence` を `true` にしてください
...
**認証方法：**
1. ブラウザウィンドウが自動で開きます（ヘッドレスではなく表示モード）
2. Instagram にログインしてください（最大10分間待機）
3. ログインが検出されると、取得したCookieを今回のInstaloader処理へ引き継ぎます
4. 既定ではセッション情報は終了時に破棄されます。永続保存が必要な場合のみ `appsettings.xml` の `AllowSocialSessionPersistence` を `true` にしてください

> **注意**: SNSセッションの永続保存と通常ブラウザCookie取り込みは既定で無効です。
...
| markitdown | ファイル変換ライブラリ（未導入時のみpipインストール） | Python site-packages |
| playwright | Webスクレイピング用ブラウザ自動化（未導入時のみpipインストール） | Python site-packages |
| httpx | X/Twitter画像ダウンロード用HTTPクライアント（未導入時のみpipインストール） | Python site-packages |
| instaloader | Instagram投稿メディアダウンロード用ライブラリ（未導入時のみpipインストール） | Python site-packages |
| openai | Instagram投稿キャプション解析用ライブラリ（未導入時のみpipインストール） | Python site-packages |
| browser-cookie3 | X/Twitter用ブラウザCookie取得ライブラリ（セッション永続保存を明示許可した場合のみ） | Python site-packages |
| pycryptodomex | ブラウザCookie復号用暗号ライブラリ（browser-cookie3 の依存として利用） | Python site-packages |
