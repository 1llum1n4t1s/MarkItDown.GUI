...
- **LLMによるMarkdown整形**: PDF・Office文書の変換結果をOllamaで自動的に整形し、きれいなMarkdownを出力
...
Ollamaを使用して、以下の2つのLLM連携機能を提供します。

- **画像説明の自動生成**: 画像ファイルをドロップすると、MarkItDownのネイティブLLM統合により画像の説明文を自動生成します
- **Markdown整形**: PDF・Office文書（`.pdf`, `.pptx`, `.ppt`, `.docx`, `.doc`）の変換結果を、Ollamaで自動的にきれいなMarkdown形式に整形します（表の再構築、見出しの付与など）
...
4. **完了**
   ...
   - PDF・Office文書をドロップすると、変換後にLLMでMarkdownが整形されます
...
| Ollama | 画像説明生成・Markdown整形 | `lib/ollama/` |
| markitdown[all] | ファイル変換ライブラリ（pip自動インストール） | Python site-packages |
| openai | OllamaのOpenAI互換API利用（pip自動インストール） | Python site-packages |
